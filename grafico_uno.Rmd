...
generate_base_pie_chart <- function(data_column_values, title_label) {
  ...
  # Genera colori per le fette del grafico (più accattivanti)
  num_categories <- n_distinct(plot_data$value_display)
  
  # Offri diverse opzioni di palette
  colors <- if (num_categories <= 12) {
    brewer.pal(max(3, num_categories), "Paired") # Utilizza una palette più vivace
  } else if (num_categories <= 8) {
    brewer.pal(max(3, num_categories), "Set2") # Un'altra buona opzione per meno colori
  }
  else {
    # Se ci sono troppe categorie, usa un gradiente ampio
    colorRampPalette(brewer.pal(8, "Dark2"))(num_categories)
  }
  
  # Assegna un colore specifico per "Non Risposto" se presente
  if ("Non Risposto" %in% levels(plot_data$value_display)) {
    na_index <- which(levels(plot_data$value_display) == "Non Risposto")
    colors_temp <- colors # Usa una copia temporanea
    colors_temp[na_index] <- "lightgrey"
    colors <- colors_temp
  }
  ...
  # Ordina i colori in base all'ordinamento delle etichette del factor
  ordered_colors <- colors[match(plot_data$value_display, levels(plot_data$value_display))]

  # Genera il grafico a torta base di R
  # Rimuovi 'main' da pie() per maggiore controllo del titolo
  pie(plot_data$count, 
      labels = plot_data$label_text, 
      col = ordered_colors, 
      cex = 0.8) # Aumenta leggermente la dimensione del testo delle etichette
  
  # Aggiungi il titolo usando 'title()' per un posizionamento più preciso
  title(main = title_label, line = -1.5, cex.main = 1) # line = -1.5 tira su il titolo
}
...
# Imposta il layout grafico con margini ancora più ridotti
# mar = c(bottom, left, top, right)
# Il valore 'top' di mar è stato ridotto per avvicinare i titoli
par(mfrow = c(rows_needed, cols_per_row), 
    mar = c(0.5, 0.5, 1.5, 0.5), # Margini interni molto ridotti, soprattutto in alto
    oma = c(0, 0, 1, 0))         # Margine esterno generale (puoi renderlo 0 se non vuoi titolo globale)
...
# Puoi aggiungere un titolo complessivo alla matrice se vuoi (optional)
# mtext("Distribuzione delle Percezioni del Questionario", side = 3, line = -0.5, outer = TRUE, cex = 1.5, font = 2)
