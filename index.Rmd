...
B2_filtered_geo <- B2[geo != "European Union (aggregate changing according to the context)"]

physical_violence_data <- B2_filtered_geo[str_detect(violence, "(?i)Physical \\(including threats\\) or sexual")]

summary_physical_violence_by_country <- physical_violence_data[, .(avg_physical_violence = mean(values, na.rm = TRUE)), by = geo][order(-avg_physical_violence)]
