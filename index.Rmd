...
CCQuesto sito è una dimostrazione di come è possibile pubblicare grafici generati con R direttamente su una pagina web. Ogni pagina contiene un grafico diverso, creato con la potenza di R e le sue librerie di visualizzazione dati.
...
grafico <- plot_ly(
  ...
  layout(
    ...
    xxaxis = list(
    title = "",
    tickangle = 0,  # <-- Etichette dritte
    automargin = TRUE,  # <-- Fa spazio in automatico
    tickfont = list(size = 10),
    tickvals = dataset$Categoria,
    ticktext = lapply(dataset$Categoria, function(x) {
      if (nchar(x) > 14) {
        paste(strwrap(x, width = 12), collapse = "<br>")  # Va a capo sulle etichette lunghe
      } else {
        x
      }
    }),
